...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Limpiar la ventana
		glClearColor(1.0f, 0.5f, 0.5f, 1.0f);	//Color rojo claro
		...
		color = glm::vec3(0.5f, 0.0f, 0.5f);    //Color Blanco
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[4]->RenderMeshGeometry();	//dibuja piramide cuadrangular
		model = modelaux;

		//Llanta izquierda delantera (Cilindro)
		model = glm::translate(model, glm::vec3(-3.8f, -0.7f, 2.65f));
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[2]->RenderMeshGeometry();	//dibuja piramide cuadrangular
		model = modelaux;

		//Llanta derecha delantera (Cilindro)
		model = glm::translate(model, glm::vec3(6.6f, 0.0f, 0.0f));
		modelaux = model;
		...
		//Llanta derecha trasera (Cilindro)
		model = glm::translate(model, glm::vec3(0.0f, -6.63f, 0.0f));
		modelaux = model;
		...
		//Llanta izquierda trasera (Cilindro)
		model = glm::translate(model, glm::vec3(-6.6f, 0.0f, 0.0f));
		modelaux = model;
		...
		model = modelaux2;
		//model = glm::translate(model, glm::vec3(3.0f, 3.0f, 3.5f));
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		color = glm::vec3(1.0f, 0.0f, 0.0f);	//Color rojo para el primer brazo
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 0.5f, 0.7f);	//Color verde grisaseo
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo 
		model = modelaux;
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 1.0f, 0.5f);	//Color verde agua
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo 
		...
		// ---------------------------------- Gato Robot ------------------------------------------------- //
		/*
		//Cuerpo del gato (Cubo)
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f));
		modelaux = model;
		modelaux2 = model;							//Referencia al cuerpo superior del gato
		model = glm::scale(model, glm::vec3(6.0f, 4.0f, 3.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 0.4f, 0.0f);		//Color verde
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		sp.render(); //dibuja esfera
		model = modelaux;
		
		//Ojo derecho
		model = glm::translate(model, glm::vec3(0.5f, 0.0f, -2.1f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.7f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 1.0f, 1.0f);		//Color blanco
		...
		model = glm::translate(model, glm::vec3(-0.5f, 0.0f, -0.25f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 0.4f, 0.0f);		//Color verde
		...
		//Boca
		model = glm::translate(model, glm::vec3(1.4f, -1.0f, 1.1f));
		modelaux = model;
		...
		color = glm::vec3(0.0f, 0.0f, 0.0f);		//Color negro
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[2]->RenderMeshGeometry(); //dibuja cono
		model = glm::rotate(model, glm::radians(-35.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		model = modelaux;

		//Oreja izquierda (Cono)
		model = glm::translate(model, glm::vec3(2.0f, 2.9f, 2.0f));
		model = glm::rotate(model, glm::radians(25.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		modelaux = model;
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion13()), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.1f, 2.0f, 0.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.3f, 0.3f, 0.3f);		//Color gris oscuro
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		...
		//Cuerpo inferior (Cubo)
		model = glm::rotate(model, glm::radians(25.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = modelaux2;
		model = glm::translate(model, glm::vec3(5.0f, 0.0f, 0.0f));
		modelaux = model;
		modelaux3 = model;							//Referencia al cuerpo inferior del gato
		...
		//Articulación superior (Cilindro)
		...
		//Parte de la pata superior (Cubo)
		model = glm::translate(model, glm::vec3(0.0f, -0.64f, 1.0f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(1.8f, 0.75f, 3.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.5f, 0.5f, 0.5f);		//Color gris
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo
		...
		//Articulación inferior (Cilindro)
		...
		//Parte de la pata inferior (Cubo)
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 2.2f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(1.8f, 0.75f, 3.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.5f, 0.5f, 0.5f);		//Color gris
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo
		...
		//Planta del pie inferior (Esfera)
		...
		color = glm::vec3(0.3f, 0.3f, 0.3f);		//Color gris oscuro
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		sp.render(); //dibuja circulo
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));	//Dejamos los ejes en sus orientaciones originales
		model = modelaux;
		//---------------------- Fin de la pata izquierda trasera ---------------------------------------

		//---------------------- Inicio de la pata derecha trasera --------------------------------------
		//Articulación superior (Cilindro)
		model = modelaux3;	//Tomamos la referencia del cuerpo inferior del gato y no de la pata anterior
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, -2.0f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		...
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));	//Dejamos los ejes en sus orientaciones originales
		model = modelaux;
		//---------------------- Fin de la pata derecha trasera ---------------------------------------

		//---------------------- Inicio de la pata derecha delantera --------------------------------------
		//Articulación superior (Cilindro)
		model = modelaux2;	//Tomamos la referencia del cuerpo superior del gato y no de la pata anterior
		...
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));	//Dejamos los ejes en sus orientaciones originales
		model = modelaux;
		//---------------------- Fin de la pata derecha delantera ---------------------------------------

		//---------------------- Inicio de la pata izquierda delantera --------------------------------------
		//Articulación superior (Cilindro)
		model = modelaux2;	//Tomamos la referencia del cuerpo superior del gato y no de la pata anterior
		model = glm::translate(model, glm::vec3(0.0f, 0.5f, 2.11f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		...
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));	//Dejamos los ejes en sus orientaciones originales
		model = modelaux;
		//---------------------- Fin de la pata izquierda delantera ---------------------------------------

		//---------------------- Inicio de la cola --------------------------------------
		//Primera articulación (Esfera)
		model = modelaux3;
		model = glm::translate(model, glm::vec3(2.1f, 0.8f, 0.0f));
		...
		//Segundo trozo de la cola
		model = glm::translate(model, glm::vec3(0.73f, -0.9f, 0.0f));
		...
		//Tercer trozo de la cola
		model = glm::translate(model, glm::vec3(0.30f, -0.8f, 0.0f));
		...
		//Cuarto trozo de la cola
		model = glm::translate(model, glm::vec3(0.23f, -0.6f, 0.0f));
		model = glm::rotate(model, glm::radians(20.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		modelaux = model;
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[2]->RenderMeshGeometry(); //dibuja cilindro
		model = glm::rotate(model, glm::radians(-170.0f), glm::vec3(0.0f, 0.0f, 1.0f));	//Rotamos los ejes a sus rotaciones originales
		model = modelaux;

		//------------------ Fin de la cola del gato --------------------------------------------------
*/
		/**/
